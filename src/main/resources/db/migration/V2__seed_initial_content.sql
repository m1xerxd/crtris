...
WITH existing AS (
    SELECT id FROM main ORDER BY id LIMIT 1
    ),
    ins AS (
...
DELETE FROM main_info WHERE main_id IN (SELECT id FROM target);

WITH target AS (SELECT id FROM main ORDER BY id LIMIT 1)
INSERT INTO main_info (name, value, position, main_id)
VALUES
    ('выпускников',        '500+', 1, (SELECT id FROM target)),
    ('выполненных проектов','100+', 2, (SELECT id FROM target)),
    ('направлений стажировки','10+',3, (SELECT id FROM target));

WITH existing AS (
    SELECT id FROM about_us ORDER BY id LIMIT 1
    ),
    ins AS (
...
DELETE FROM company_value WHERE about_us_id IN (SELECT id FROM target);

WITH target AS (SELECT id FROM about_us ORDER BY id LIMIT 1)
INSERT INTO company_value (title, position, about_us_id)
VALUES
    ('Ответственность',        1, (SELECT id FROM target)),
...
DELETE FROM education_item;

INSERT INTO education_item (title, description, position)
VALUES
    ('Разработка', 'Какое-то краткое описание', 1),
...
VALUES
    ('Доступная библиотека обучающих курсов по hard и soft', 1),
...
VALUES
    ('Марина С.', 'Мы использовали пакет «Команда под ключ» для масштабирования нашего проекта. Это было решение, которое сэкономило нам массу времени и усилий... ', 'HR-менеджер компании "SoftSolutions"', 1),
    ('Марина С.', 'Мы использовали пакет «Команда под ключ» для масштабирования нашего проекта. Это было решение, которое сэкономило нам массу времени и усилий... ', 'HR-менеджер компании "SoftSolutions"', 2),
    ('Марина С.', 'Мы использовали пакет «Команда под ключ» для масштабирования нашего проекта. Это было решение, которое сэкономило нам массу времени и усилий... ', 'HR-менеджер компании "SoftSolutions"', 3);

DELETE FROM faq_item;

INSERT INTO faq_item (question, answer, position)
VALUES
    ('Какой-то вопрос?', 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum ut orci eu sapien eleifend viverra condimentum et tortor. ', 1);

DELETE FROM contacts;

INSERT INTO contacts (email, phone_number, telegram)
VALUES
    ('fkfkffof@gmail.com', '+4994404', 'kgjgjgg');
